...
‒ class balancing, speaker-exclusive 60/20/20 split  
...
BATCH_SIZE = 32
EPOCHS     = 50          # can be high; early-stop will cut it
LR         = 2e-4
VAL_P      = 0.20
TEST_P     = 0.20
...
class MFCC2D(Dataset):
    def __init__(self, fpaths, ys, mu, sigma, H, W, augment=False):
        self.fpaths, self.ys = fpaths, torch.tensor(ys, dtype=torch.float32).unsqueeze(1)
        self.mu, self.sigma, self.H, self.W, self.augment = mu, sigma, H, W, augment
    ...
    def __getitem__(self, i):
        ...
        if x.shape != (self.H, self.W):                         # pad / crop
        ...
        x = ((x - self.mu) / self.sigma).astype(np.float32)     # normalise & cast
...
BEST_MODEL = os.path.join(MODEL_DIR, "resNet_3_best.pth")
FINAL_MODEL = os.path.join(MODEL_DIR, "resNet_3.pth")
CURVE_FILE = os.path.join(CURVE_DIR, "resNet_3.png")
...
for epoch in range(1, EPOCHS+1):
    ...
    tloss /= len(train_loader.dataset); train_losses.append(tloss)
    ...
    vloss /= len(val_loader.dataset); val_losses.append(vloss)
...
# ── AFTER TRAINING ──────────────────────────────────────────────────────
torch.save(model.state_dict(), FINAL_MODEL); print("Final model saved")

plt.figure(figsize=(6,4))
plt.plot(train_losses, label="train"); plt.plot(val_losses, label="val")
plt.xlabel("Epoch"); plt.ylabel("Loss"); plt.title("Loss curve")
plt.legend(); plt.grid(True); plt.tight_layout(); plt.savefig(CURVE_FILE)
